...
Page({
    ...
    onLoad: function (options) {
        ...
        request({
            ...
            fail: function (e) {
                that.goErrorPage();
                reportErr(
                    encodeURIComponent('四级地址选择首次数据onload请求request异常，具体信息：') +
                        e.errMsg,
                );
            },
        });
    ...
    resetChoice: function (e) {
        ...
        request({
            ...
            fail: function (e) {
                that.goErrorPage();
                reportErr(
                    encodeURIComponent('四级地址选择resetChoice数据请求request异常，具体信息：') +
                        e.errMsg,
                );
            },
        });
    ...
    getSpecificList: function (e) {
        ...
        var objTarget = e.currentTarget,
            ...
            url =
                app.globalRequestUrl +
                that.data.pDir +
                '/norder/select' +
                that.getDistrictType(iAddressType) +
                '.action',
        ...
        if (iAddressType > 1) {
            url +=
                '?id' +
                that.getDistrictType(iAddressType - 1) +
                '=' +
                arrHeader[iAddressType - 2].regionId;
        }
        ...
        request({
            ...
            success: function (data) {
                ...
                (tempObj.scrollTop = Math.random() * 0.00001), that.setData(tempObj);
            },
            fail: function (e) {
                reportErr(
                    encodeURIComponent(
                        '四级地址选择getSpecificList数据请求request异常，具体信息：',
                    ) + e.errMsg,
                );
            },
        });
    ...
    dealWithList: function (e) {
        ...
        var url =
            ...
            that.data.pDir +
            '/norder/select' +
            that.getDistrictType(iRegionType + 1) +
            '.action?id' +
            that.getDistrictType(iRegionType) +
            '=' +
            objCur.dataset.id;
        if (iRegionType >= 4) {
        ...
        request({
            ...
            fail: function (e) {
                reportErr(
                    encodeURIComponent('四级地址选择dealWithList数据请求request异常，具体信息：') +
                        e.errMsg,
                );
            },
        });
        let eventIdObj = [
            'WProductDetail_AddressFChooseO',
            'WProductDetail_AddressFChooseS',
            'WProductDetail_AddressFChooseT',
        ];
        // log.click({
    ...
    toGenerateDisplayAddress: function () {
        ...
        var url =
            ...
            that.data.pDir +
            '/norder/select' +
            that.getDistrictType(typeAccount) +
            '.action?id' +
            that.getDistrictType(typeAccount - 1) +
            '=' +
            lastAddressId;
        ...
        request({
            ...
            fail: function (e) {
                reportErr(
                    encodeURIComponent(
                        '四级地址选择toGenerateDisplayAddress数据请求request异常，具体信息：',
                    ) + e.errMsg,
                );
            },
        });
